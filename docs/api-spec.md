...
**7/10 MVP 단순화 (D-022)**: FastAPI가 보내는 결과 PATCH는 **전부 `DONE`으로 처리** — `ANALYZING`·`FAILED` 전환은 후순위(§5.2).
...
| Method | Path | 용도 | 상태 | 상세 |
| --- | --- | --- | --- | --- |
| POST | `/api/internal/reports` | 번들 및 분석 저장 (report+3종, 1 트랜잭션) | ✅ 기본 | §5.1 |
...
- `type`: 장애 유형 — **LLM 판정** (7/10 확정, D-022). 전달·저장 경로(PATCH 바디 위치·컬럼화)는 §6 쟁점 3
...
- `detectedAt`: 감지 시각 = `trigger.timestamp` (프론트 `time` → 이름 통일)
...
```json
{
  ...
  "title": "Svc_Kill_Media_20251104_000111",
  "window": { "start": "2025-11-04 00:00:00.000", "end": "2025-11-04 00:10:00.000" },
  "trigger": {
    "timestamp": "2025-11-04 00:01:57.490",
    "modality": ["log"]
  },
...
- `trigger` (D-021): **감지 시각(`timestamp`)과 트리거가 걸린 모달리티(`modality[]`, 값: `log`/`metric`/`trace`, 복수 가능)만 전달** — signal 상세·서비스명은 계약에서 제외. 트리거=이상 감지 / LLM=원인 판단 분리(D-020)의 계약 확장 — 감지 신호·의심 서비스를 LLM에 넘기면 정답 유출 소지. 목록 `detectedAt`의 원천 (§2)
...
**에러**: `422` 스키마 위반(timestamp 형식·필수 필드), `409` 중복 `bundle_id`(Spring 응답 그대로 전달, 기존 `report_id` 포함), `502` Spring 저장 실패(report 미생성 — 엣지가 재시도)
...
```json
{
  "report": {
    ...
    "trigger": { "timestamp": "2025-11-04 00:01:57.490", "modality": ["log"] }
  },
...
- `report` = 목록 아이템 + `windowStart/windowEnd` + `trigger`
...
### 5.1 POST /api/internal/reports — 번들 및 분석 저장

요청 바디 = **`/ingest`(§3.1)와 동일 계약** (bundle_id·title·window·trigger·3종). report 생성 + 3종 insert를 **한 트랜잭션**으로.
`detectedAt`은 Spring이 `trigger.timestamp`에서 파생 (별도 필드 아님). `type`은 LLM 판정(D-022), `service` 원천은 §6 쟁점 3.

**Response** `201` `{ "report_id": 1 }`

**에러**: `409` 중복 `bundle_id` — `{ "error": { "code": "DUPLICATE_BUNDLE", "message": "...", "reportId": 1 } }` (DB UNIQUE 제약이 최종 방어선), `422` 필수 필드·timestamp 형식 위반, `500` 트랜잭션 롤백(report 미생성)
...
- **7/10 MVP 단순화 (D-022)**: FastAPI가 보내는 결과 PATCH는 **전부 `DONE`으로 처리** — `ANALYZING`·`FAILED` 전환은 후순위. ~~`DONE` 시 `severity` 필수(v0.2)~~ → `severity`는 컬럼·계약만 만들어 두고 **필수 검증 완화**(누락 시 NULL 허용, LLM 판정 연동은 후속)
- 상태 전이 검증은 최소만: `DONE` ⇒ `result` 필수. 역방향 전이 차단 같은 상태기계는 필요해지면 추가

## 6. 회의 쟁점 (우선순위순)

1. **`report.severity` 컬럼 신설** — `HIGH/MID/LOW`, NULL 허용(분석 전). 원천은 LLM 판정, FastAPI가 `PATCH DONE`에 포함(§5.2). 대시보드 `highCount`·목록 severity 필터가 여기 걸림. **스키마 변경 + internal API 계약 변경 → 가희와 확인** · 7/10: 컬럼·계약만 신설하고 필수 검증은 완화, LLM 판정 연동은 후속 (D-022)
...
3. **목록 `service` 원천 + `type` 전달 경로 (Q-007 잔여)** — `type`은 **LLM 판정으로 확정**(7/10, D-022). 남은 것: ① `type`의 전달·저장 경로 — severity처럼 `PATCH DONE` 바디 최상위 vs `result` 내부, 컬럼화 여부 ② `type` 값 체계(`Svc_Kill`·`Code_Stop`·`Perf_CPU` 등) ③ `service`의 새 원천 — D-021로 trigger에서 서비스명 제거, 파생 불가 (유경·에이전트팀)
...
6. **timestamp 규약** `yyyy-MM-dd HH:mm:ss.SSS` UTC — SDK↔FastAPI 계약(유경·예지)과 동일한지 확인 (기존 안건 유지). 7/10 필드명 `ts`→`timestamp` 전면 개명(D-022) — SDK 쪽 필드명도 동일한지 확인 필요
