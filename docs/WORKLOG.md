...
- 11:55 **코드리뷰 2차 지적 4건 대응 (SVC-01)** — ① dedup 재조회를 `REQUIRES_NEW` 별도 빈(`TriggerDedupLookup`)으로 분리 — 1차 때 "모든 DIVE 409 금지"로 넣었던 catch 내 재조회가 rollback-only 트랜잭션 읽기 위험이라 2차에서 지적됨. 두 지적을 동시 만족하는 유일 해법(새 트랜잭션=오염 회피+판별 유지) ② `status` DONE-only 강제(v0.2.1) — 임의 status가 조회 안 되는 유령 리포트 되던 것 ③ signal `service` 128자(VARCHAR 한도) 초과 저장 전 422 ④ 테스트: escape·KST는 1차(`bb7669f`)에 이미 커버됨을 확인, DONE-only 순수 유닛 추가(dedup race·length는 DB필요→curl). 실배포 회귀: OPEN→422·service129→422·정상201·재전송409 통과, REQUIRES_NEW 경로는 로직리뷰+빈주입(clean부팅) 검증. 커밋 `29c6fba`(fix)·`08e8d4a`(test)
