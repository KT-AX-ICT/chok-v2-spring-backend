...
`report` 부모에는 `bundle_id`(엣지 멱등 키, UNIQUE) · `trigger_info`(발화 신호 JSON) · `window_from/to` · `status` · `severity` · `result`가 있다 — [`schema.sql`](./schema.sql) 참조.
목록의 `type`·`service`·`detectedAt`은 별도 컬럼 없이 `trigger_info`에서 파생 — MVP 규모(report 수백 건)에선 조회 시 파생으로 충분, 정렬·검색 병목이 생기면 그때 ALTER로 컬럼화.
