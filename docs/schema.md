...
`report` 부모에는 `bundle_id`(엣지 멱등 키, UNIQUE) · `trigger_info`(발화 신호 JSON `{timestamp, modality[]}`) · `window_from/to` · `status` · `severity` · `result`가 있다 — [`schema.sql`](./schema.sql) 참조.
목록의 `detectedAt`은 별도 컬럼 없이 `trigger_info.timestamp`에서 파생 — MVP 규모(report 수백 건)에선 조회 시 파생으로 충분, 정렬·검색 병목이 생기면 그때 ALTER로 컬럼화. `type`·`service`는 7/10 trigger 축소(signal·서비스명 제거, D-021)로 **원천 재확정 필요** (api-spec §6 쟁점 3, Q-007).
...
| T6 | `PATCH status=DONE+severity+result` → 상세에 detail·severity 노출, `DONE` 아니면 프론트 조회 404 | status 전이 + severity·result 계약 (api-spec v0.2.1) |
